...
public class ModuleDiceCmd extends BaseGroupModule {
	...
	public boolean onGroupMessage(GroupMessageEvent gme) {
        ...
        try {
            ...
            switch (iter.next()) {
                case "r":
                    entity.sjfTx.sendGroupMessage(groupId, String.format("%s投掷%s:D100 = %d", entity.configManager.getNickName(groupId, qqId), iter.hasNext() ?iter.next(): "", random.nextInt(100)));
                    return true;
                case "ra":
                    String ras = iter.next();
                    entity.sjfTx.sendGroupMessage(groupId, String.format("%s进行检定:D100 = %d/%s", entity.configManager.getNickName(groupId, qqId), random.nextInt(Integer.parseInt(ras)), ras));
                    return true;
                case "li":
                    entity.sjfTx.sendGroupMessage(groupId, String.format("%s的疯狂发作-总结症状:\n1D10=%d\n症状: 狂躁：调查员患上一个新的狂躁症，在1D10=%d小时后恢复理智。在这次疯狂发作中，调查员将完全沉浸于其新的狂躁症状。这是否会被其他人理解（apparent to other people）则取决于守秘人和此调查员。\n1D100=%d\n具体狂躁症: 臆想症（Nosomania）：妄想自己正在被某种臆想出的疾病折磨。(KP也可以自行从狂躁症状表中选择其他症状)", entity.configManager.getNickName(groupId, qqId), random.nextInt(11), random.nextInt(11), random.nextInt(101)));
                    return true;
                case "ti":
                    entity.sjfTx.sendGroupMessage(groupId, String.format("%s的疯狂发作-临时症状:\n1D10=%d\n症状: 逃避行为：调查员会用任何的手段试图逃离现在所处的位置，状态持续1D10=%d轮。", entity.configManager.getNickName(groupId, qqId), random.nextInt(11), random.nextInt(11)));
                    return true;
                case "rd":
                    entity.sjfTx.sendGroupMessage(groupId, String.format("由于%s %s骰出了: D100=%d", iter.next(), entity.configManager.getNickName(groupId, qqId), random.nextInt(101)));
                    return true;
                case "nn":
                    if (!iter.hasNext()) {
                        entity.configManager.setNickName(qqId, null);
                        entity.sjfTx.sendGroupMessage(groupId, "我以后会用你的QQ昵称称呼你");
                        return true;
                    }
                    String name = iter.next();
                    if (name.length() > 30) {
                        entity.sjfTx.sendGroupMessage(groupId, "太长了,记不住");
                        return true;
                    }
                    entity.configManager.setNickName(qqId, name);
                    entity.sjfTx.sendGroupMessage(groupId, "我以后会称呼你为" + name);
                    return true;
                case "jrrp":
                    float fpro = 0f;
                    if (c == '0') {
                ...
                case "roll":
                    switch (iter.next()) {
                        ...
                        case "pl":
                        case "player":
                            entity.sjfTx.sendGroupMessage(groupId, thData.randomPlane(iter.next()));
                            return true;
                    }
                    return true;
                case "draw":
                    ...
                    switch (drawcmd) {
                        case "spell":
                            if (list.size() == 3) {
                                entity.sjfTx.sendGroupMessage(groupId, thData.randomSpell().name);
                            } else if (list.size() == 4) {
                        ...
                        case "ufo":
                            ...
                            if (ufor < 8) {
                                ...
                                entity.sjfTx.sendGroupMessage(groupId, entity.image(new File(entity.appDirectory + "ufo/" + fileName[ThreadLocalRandom.current().nextInt(3)]), groupId), entity.image(new File(entity.appDirectory + "ufo/" + fileName[new Random().nextInt(3)]), groupId), entity.image(new File(entity.appDirectory + "ufo/" + fileName[new Random().nextInt(3)]), groupId));        
                            } else if (ufor == 8) {
                                entity.sjfTx.sendGroupMessage(groupId, entity.image(new File(entity.appDirectory + "ufo/yellow.gif"), groupId));
                            } else {
                                entity.sjfTx.sendGroupMessage(groupId, entity.image(new File(entity.appDirectory + "ufo/colorful.gif"), groupId));
                            }
                            return true;
                        case "all":
                            ...
                            return true;            
                        default:
                            entity.sjfTx.sendGroupMessage(groupId, "可用.draw help查看帮助");
                    }
                    return true;
                case "spellInfo":
            ...
            entity.sjfTx.sendGroupMessage(groupId, "参数错误");
        }
        return false;
	}
