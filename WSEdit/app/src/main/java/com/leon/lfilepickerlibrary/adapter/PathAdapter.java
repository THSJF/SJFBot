...
public class PathAdapter extends RecyclerView.Adapter<PathAdapter.PathViewHolder> {
    ...
    private static WeakHashMap<File,BitmapDrawable> cache = new WeakHashMap<>();
    ...
    private void updateFileIconStyle(ImageView imageView, File file) {
        if (isPicture(file)) {
            BitmapDrawable bd = cache.get(file);
            if (bd == null) {
                bd = new BitmapDrawable(ThumbnailUtils.extractThumbnail(BitmapFactory.decodeFile(file.getAbsolutePath()), 250, 250));
                cache.put(file, bd);
            }
